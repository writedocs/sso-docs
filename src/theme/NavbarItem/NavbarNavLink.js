...
export default function NavbarNavLink({
  ...
  const isValid = configNavbarItemsOriginal.every(
    (item) => possibleValues.includes(item) || navbar.find((i) => i.label.toLowerCase() === item)?.icon
  );
  ...
  if (label && typeof label === "string") {
    ...
    const currentItem = navbar.find((item) => item.label.toLowerCase() === originalLabel);
  ...
  const linkContentProps = html
    ...
    : {
        children: (
          <>
            {displayIcon && Icon && <Icon weight="bold" className={iconClass} />}
            {label}
